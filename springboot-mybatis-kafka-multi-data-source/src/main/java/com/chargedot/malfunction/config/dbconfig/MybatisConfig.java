...
import com.chargedot.malfunction.util.SpringContextUtil;
...
@MapperScan(basePackages = "com.chargedot.malfunction.dao.mapper")
public class MybatisConfig {

    private static Logger LOG = LoggerFactory.getLogger(MybatisConfig.class);

    //从库的数量，当配置多个读库的时候能够进行简单的负载均衡
...
//            sessionFactoryBean.setPlugins(plugins);
            ...
            LOG.info("mybatis resolver mapper*xml is error", e);
            return null;
        } catch (Exception e) {
            LOG.info("mybatis sqlSessionFactoryBean create error", e);
            return null;
        }
    }
...
//                    LOG.info("======use write databases=====");
                    return DataSourceType.write.getType();
                }

                //读库， 简单负载均衡,配置多个读库的时候用
//                int number = count.getAndAdd(1);
//                int lookupKey = number % readSize;
//                LOG.info("======use read databases=====");
